 ...
 * ```
 *
 * Returns current borrow capacity where the amount of the `0x123` asset has been affected as `balance - 100000000n`.
 */
export function useUserBorrowCapacity(args?: UseBorrowCapacityValueArgs) {
  const {
    changes = {}
  } = args ?? {};
  ...
  return collateralAssets.reduce((totalCapacity, collateral) => {
    ...
    const borrowCapacityValue = (balanceValue * collateral.collateralFactor) / BASE_FACTOR;
